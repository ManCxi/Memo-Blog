...
- **独立页面管理**：支持普通 Markdown/富文本排版，并支持 **原生自定义 HTML 代码页面模式**（自适应 iframe 渲染且自动撑开高度，与站点顶栏底栏完美对齐，支持手机端完美自适应）
- **附件库**：支持本地上传、图片压缩、多选删除、同步本地目录
- **站点设置**：站点信息、导航菜单配置、轮播图管理、友情链接
- **前台能力**：归档、搜索、分类页、标签页、友情链接页
- **Feed 能力**：`/rss.xml`、`/sitemap.xml`
- **一键备份与恢复**：支持打包导出整站数据（包括数据库全部表数据与 `/uploads` 目录）为 `.zip` 压缩包，支持一键无缝还原，跨 SQLite/MySQL/PostgreSQL 数据库完美兼容
...
- **工具库**：`marked`, `zod`, `slugify`, `adm-zip` (纯 JS 的 Zip 压缩库)
